...
// Dynamically generate 50 unique high-quality images to completely fill the dome 
// and eliminate any empty, transparent gaps.
...
function buildItems(pool, seg) {
  ...
  // Ensure no adjacent duplicates if possible
...
export default function DomeGallery({
  ...
  grayscale = true,
  autoRotateSpeed = 0.05 // Added a speed prop for the slow rotation
}) {
  ...
  useEffect(() => {
    const renderLoop = () => {
      // Only auto-rotate if the user is NOT dragging, NOT interacting, and NO inertia is active
      ...
      autoRotateRAF.current = requestAnimationFrame(renderLoop);
    };
    autoRotateRAF.current = requestAnimationFrame(renderLoop);
  ...
  const openItemFromElement = useCallback(el => {
      ...
      const validSx0 = Math.max(tileR.width / frameR.width, 0.001);
      const validSy0 = Math.max(tileR.height / frameR.height, 0.001);
      ...
      if (openedImageWidth || openedImageHeight) {
        overlay.addEventListener('transitionend', function onFirstEnd(ev) {
