...
import { motion } from 'framer-motion'; // <-- NEW: Framer Motion imported
...
const containerVariants = {
  ...
  show: {
    opacity: 1,
    transition: { staggerChildren: 0.1 } // This creates the "waterfall" effect
  }
};
        ...
        <div className="p-8 max-w-7xl mx-auto overflow-hidden">
          ...
          >
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <MetricCard title="Next Class" value="Physics" subtitle="in 15 mins" icon={Clock} colorClass="from-blue-500 to-cyan-500" />
              <MetricCard title="Overall Grade" value="A-" subtitle="Mid-Term" icon={Award} colorClass="from-emerald-500 to-teal-500" />
              
              {/* UPDATE: Attendance Card to include Timetable Link */}
              <GlassCard hover className="relative overflow-hidden group h-full">
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  {/* NEW: View Timetable Button */}
                  {studentData?.class?.timetableUrl && (
                    <a 
                      href={studentData.class.timetableUrl} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-xs font-medium text-white transition-colors border border-white/10"
                    >
                      View Timetable
                    </a>
                  )}
                ...
                <div className="flex items-baseline space-x-2">
                  <p className="text-3xl font-semibold text-white tracking-tight">94%</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="p-4 rounded-xl border border-rose-500/30 bg-rose-500/5 hover:bg-rose-500/10 transition-colors">
                        <div className="flex justify-between items-start mb-2">
                          <span className="text-xs font-medium px-2 py-1 rounded bg-rose-500/20 text-rose-400">Due Tomorrow</span>
                        </div>
                      ...
                      <div className="p-4 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 transition-colors">
                        <div className="flex justify-between items-start mb-2">
                          <span className="text-xs font-medium px-2 py-1 rounded bg-white/10 text-gray-300">Due in 3 days</span>
                        </div>
                        <h4 className="font-medium text-gray-200 mb-1">English Essay</h4>
                        <p className="text-sm text-gray-500 mb-4">1000 words on Shakespeare's tragedies.</p>
                        <button className="w-full py-2 rounded-lg bg-white/5 hover:bg-white/10 text-white text-sm font-medium transition-colors">
                          Submit Now
                        </button>
                      </div>
                      ...
                      <>
                        ...
                        <div className="mb-6">
                          ...
                          <p className="text-sm text-rose-400 flex items-center">
                            <AlertCircle size={14} className="mr-1" /> Due {new Date(studentData.fees[0].dueDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}
                          </p>
                        </div>
                ...
                <motion.div variants={itemVariants}>
                  <GlassCard>
                    ...
                    <div className="space-y-4 relative before:absolute before:inset-0 before:ml-[11px] before:h-full before:w-0.5 before:bg-white/10">
                      {[
                        { time: "09:00 AM", subject: "Advanced Mathematics", room: "Room 302", active: false },
                        { time: "11:30 AM", subject: "Quantum Physics", room: "Science Lab 1", active: true },
                        { time: "01:00 PM", subject: "Lunch Break", room: "Cafeteria", active: false },
                        { time: "02:00 PM", subject: "World History", room: "Room 105", active: false },
                      ].map((cls: any, i: number) => (
                        <div key={i} className="relative flex items-start pl-8 group">
                          <div className={`absolute left-0 top-1.5 w-6 h-6 rounded-full border-4 border-black flex items-center justify-center ${cls.active ? 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.8)]' : 'bg-gray-600'}`} />
                          <div className={`p-3 rounded-xl border w-full transition-colors ${cls.active ? 'bg-blue-500/10 border-blue-500/30' : 'bg-white/5 border-transparent hover:border-white/10'}`}>
                            <div className="flex justify-between items-center mb-1">
                              <h4 className={`font-medium text-sm ${cls.active ? 'text-blue-400' : 'text-gray-200'}`}>{cls.subject}</h4>
