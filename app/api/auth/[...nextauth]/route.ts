...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    // THIS IS THE MAGIC: It intercepts Google Logins!
    async signIn({ user, account }) {
      if (account?.provider === 'google' && user.email) {
        // 1. Check if this Google email is already in our database
        const existingUser = await prisma.user.findUnique({
          where: { email: user.email }
        });

        // 2. If they don't exist, AUTO-CREATE their entire student profile!
        if (!existingUser) {
            ...
            data: {
              ...
              studentProfile: {
                create: {
                  ...
                  dateOfBirth: new Date('2010-01-01'), // Placeholder birthdate
                  gradeLevel: '10th', // Default grade
                  section: 'A',
                  // Give them a welcome fee automatically!
                  fees: {
                    create: {
