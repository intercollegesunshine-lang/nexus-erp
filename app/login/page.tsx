...
export default function LoginPage() {
  ...
  <div className="bg-transparent backdrop-blur-md rounded-xl px-4 md:px-6 py-3 flex items-center space-x-10">
    ...
    <div className="hidden sm:flex space-x-8 text-base font-bold text-black">
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 mt-8 md:mt-12 w-full max-w-5xl mx-auto text-center pb-12">
        ...
        <div className="mt-10 sm:mt-12 w-full max-w-md bg-transparent border border-white/20 rounded-[2rem] p-6 sm:p-8 shadow-[0_16px_40px_-12px_rgba(27,19,60,0.15)] text-left mx-auto">
  <button
    type="button"
    onClick={handleGoogleLogin}
    disabled={isGoogleLoading || isLoading}
    className="w-full flex items-center justify-center space-x-3 bg-transparent border border-white/30 text-[#1B133C] font-semibold py-3.5 rounded-xl hover:bg-transparent transition-colors mb-6 shadow-sm"
  >
    {isGoogleLoading ? (
  ...
  <div className="flex items-center space-x-4 mb-6">
    ...
    <span className="text-xs text-[#1B133C]/50 font-bold uppercase tracking-wider">
      Or Use Email
    </span>
    <div className="flex-1 h-px bg-[#1B133C]/10"></div>
  </div>
  ...
  <form onSubmit={handleLogin} className="space-y-5">
    <div className="space-y-1.5">
      <label className="text-sm font-semibold text-[#1B133C]/80 ml-1">
    ...
    <button
      ...
      className="w-full mt-2 relative flex items-center justify-center space-x-2 bg-transparent border border-white/30 px-6 py-3.5 rounded-xl text-sm font-semibold text-[#1B133C] shadow-[0px_4px_12px_rgba(0,0,0,0.15)] hover:shadow-[0px_6px_16px_rgba(0,0,0,0.2)] transition-all duration-300 disabled:opacity-70 group"
    >
      {isLoading ? (
        <div className="w-5 h-5 border-2 border-[#1B133C]/30 border-t-[#1B133C] rounded-full animate-spin" />
      ) : (
