...
import { LayoutDashboard, BookOpen, CreditCard, Award, Calendar, Settings, Menu, X, FileText, LogOut, Download } from 'lucide-react';
...
export default function AcademicsPage() {
  ...
  const handleDownload = () => {
    if (studentData) generateTranscriptPDF(studentData);
  };
      ...
      <main className="flex-1 h-screen overflow-y-auto relative z-10 p-6 md:p-10 scroll-smooth">
        <div className="flex items-center justify-between mb-8 md:mb-12 sticky top-0 z-30 pt-4 pb-4 bg-[#0c0c0c]/80 backdrop-blur-xl -mx-6 px-6 md:-mx-10 md:px-10 border-b border-white/5">
           <div className="flex items-center gap-4">
             ...
             <span className="font-semibold text-sm tracking-wide hidden sm:block text-white/80">Transcripts</span>
           </div>
           <img src="/logo.png" alt="Logo" className="w-8 h-8 object-contain opacity-80" />
        </div>

        <div className="max-w-4xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
            <div>
              <h1 className="text-4xl md:text-6xl font-semibold tracking-tight text-white mb-2">Transcripts.</h1>
              <p className="text-white/50 text-sm font-medium">Your complete academic history and examination results.</p>
            </div>
            {studentData.results?.length > 0 && (
              <button onClick={handleDownload} className="inline-flex items-center justify-center gap-2 rounded-full bg-white text-black font-semibold text-sm px-6 py-3.5 hover:bg-white/90 active:scale-95 transition-all shadow-[0_0_15px_rgba(255,255,255,0.2)]">
           ...
           {studentData.results?.length > 0 ? studentData.results.map((exam: any, i: number) => (
             <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.1 }} key={i} className="liquid-glass rounded-3xl p-6 md:p-8 flex flex-col md:flex-row justify-between md:items-center gap-6 group hover:bg-white/[0.04] transition-colors">
                <div className="flex items-center gap-5">
                  <div className="p-4 rounded-2xl border border-white/10 bg-white/5 group-hover:border-[#00d2ff]/30 transition-colors"><FileText size={24} className="text-[#00d2ff]" /></div>
                  <div>
                    <h3 className="font-semibold text-xl text-white mb-1">{exam.subject}</h3>
                    <p className="text-white/40 text-sm font-medium">{exam.examName} • {new Date(exam.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
                <div className="text-right border-t md:border-0 border-white/5 pt-4 md:pt-0">
                    <p className="text-4xl font-bold tracking-tight text-white">{exam.marksObtained}<span className="text-xl text-white/30 font-medium">/{exam.totalMarks}</span></p>
                    <p className="text-xs font-bold tracking-widest uppercase mt-2 text-[#28c840] bg-[#28c840]/10 inline-block px-3 py-1 rounded-full">Grade {exam.grade}</p>
                </div>
             </motion.div>
           )) : (
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="liquid-glass rounded-3xl p-16 text-center">
                <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mx-auto mb-4 border border-white/10"><Award size={32} className="text-white/40" /></div>
                <h3 className="text-lg font-semibold text-white mb-1">No Records Found</h3>
                <p className="text-white/50 text-sm">Your academic results will appear here once published.</p>
             </motion.div>
           )}
          </div>
        </div>
